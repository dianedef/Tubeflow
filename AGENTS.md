...
- Follow root README steps:
  ...
  - Optional `OPENAI_API_KEY` for summaries (Convex action uses it).
...
- Ensure Convex, Clerk, and (optionally) OpenAI env vars are set before running backend `setup`/deploy; missing `OPENAI_API_KEY` will store an error string in summaries.
